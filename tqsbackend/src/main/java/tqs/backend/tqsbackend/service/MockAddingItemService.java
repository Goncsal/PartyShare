...
public class MockAddingItemService {
    ...
    public void init() {
        if (userRepository.count() == 0 && itemRepository.count() == 0) {
            // Create owner users
            User owner1 = new User("Maria Silva", "maria@ua.pt", BCrypt.hashpw("password123", BCrypt.gensalt()),
                    UserRoles.OWNER);
            User owner2 = new User("João Santos", "joao@ua.pt", BCrypt.hashpw("password123", BCrypt.gensalt()),
                    UserRoles.OWNER);
            ...
            Item partyLamps = new Item("Party Lights", "Beautiful Colorful Lights", 55.0, lighting, 0.0, "Lisbon");
            partyLamps.setOwnerId(owner1.getId());
            itemRepository.save(partyLamps);

            Item mealTable = new Item("Party Meal Table", "Foldable dining table", 150.0, furniture, 0.0, "Porto");
            mealTable.setOwnerId(owner1.getId());
            itemRepository.save(mealTable);

            Item audioInterface = new Item("Audio Interface", "High performance laptop", 1200.0, electronics, 0.0,
                    "Lisbon");
