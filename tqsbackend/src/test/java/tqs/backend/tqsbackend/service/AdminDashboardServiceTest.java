...
class AdminDashboardServiceTest {
    ...
    void getGlobalStats_shouldReturnCorrectStats() {
        ...
        Booking b1 = new Booking(); b1.setTotalPrice(new BigDecimal("100.50")); b1.setStatus(BookingStatus.ACCEPTED);
        Booking b2 = new Booking(); b2.setTotalPrice(new BigDecimal("50.25")); b2.setStatus(BookingStatus.ACCEPTED);
        // Assuming the service filters by status or sums all. Let's assume it sums COMPLETED bookings for revenue.
        // Or if the requirements are simpler, just all bookings. 
        // For a sophisticated dashboard, we usually sum revenue of *paid/completed* bookings.
        // I will implement the service to sum only completed/accepted bookings if possible, or assume simple sum for now.
        // Let's refine the test to expect revenue from ALL bookings for now, or Mock a specific repository method.
        // Ideally we should have a custom query in repository for revenue relative to status.
        // Checking repositories capabilities...
        // For this iteration, I'll assume the service fetches all bookings and sums them up, or uses a repo method.
        // Let's assume the service calculates it from a list or a count.
        // Better: Mock a custom query `bookingRepository.sumTotalPriceByStatus(BookingStatus.COMPLETED)`
        ...
        // Let's go with findAll() for simplicity unless performance is key (it is, but let's start simple).
        ...
        // To make it compile without changing Repo interfaces yet, I'll use standard JPA methods if possible, 
        // or accept I need to add methods to Repos.
        // Adding methods to Repos is standard.
        // So I will Mock `bookingRepository.sumTotalRevenue()` and `userRepository.findAverageUserRating()`.
        
        // Since I can't easily mock methods that don't exist in the interface class literal in the test without casting or using spies on interfaces (complex),
        // I will assume standard methods or that I will add them. 
        // For the sake of this test compiling *now* (strict TDD), I should probably rely on existing methods or `findAll`.
        ...
        // Use a mocked Double for the average directly if possible via custom query later.
        ...
        /* 
           ...
           - Revenue: Sum of all bookings (simplified) or filtered. I'll simply sum all bookings in the test.
